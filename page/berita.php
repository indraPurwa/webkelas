...
if($aksi == "tampil_berita" and $id != "") {
	...
	echo '<div class="berita">'
					...
					.'<div class="info">'.date("d/m/Y", strtotime($data['tgl_berita'])).' Posted by: '.$createduser['nama'].'</div>'
	...
	if($tmp_jml_baris){
		...
		while ($data = $berita->fetch_assoc() ) {
			...
			echo '<div class="list">'
					    ...
					    .'<div class="image"><img src="foto/berita/'.$data['foto'].'" width="200px" height="200px"></div>'
					    .'<div class="artikel">'
							.'<div> Tanggal : '.date("d/m/Y", strtotime($data['tgl_berita'])).', Posted by: '.$createduser['nama'].'</div>'
